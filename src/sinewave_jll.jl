...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import sinewave_jll on Julia versions older than 1.3!")
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("9026e745-461c-5eb7-b50a-ddb23c07924d"))
platforms = [Pkg.Artifacts.unpack_platform(e, "sinewave", artifacts_toml) for e in artifacts["sinewave"]]
...
if best_platform === nothing
    @debug("Unable to load sinewave; unsupported platform $(triplet(platform_key_abi()))")
else
    # Load the appropriate wrapper
    include(joinpath(@__DIR__, "wrappers", "$(best_platform).jl"))
end

end  # module sinewave_jll
